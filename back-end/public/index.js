...
function refreshList(){
    ...
    if (window.location.pathname == "/users.html"){
        ...
        fetch(url + "/api/data/users?size=" + Number.MAX_SAFE_INTEGER + "&hash=" + sessionStorage.getItem('logintoken'))
        .then(response => response.json())
        .then(data => {
            data.forEach(element => {
                ...
                <div class="d-flex justify-content-between align-items-center py-2">
                  ...
                  <div>
                    <i class="icon-edit">
                      <button type="button" class="btn btn-primary" id="iconButton" data-bs-toggle="modal" data-bs-target="#EditModal" onclick="startEditUser('${element.token}')">
                    ...
                    <i class="icon-trash mx-4">
                      <button type="button" class="btn btn-primary" id="iconButton" data-bs-toggle="modal" data-bs-target="#DeleteModal" onclick="startDeleteUser('${element.token}')">
        ...
        if (window.location.pathname == "/machines.html"        ){
          ...
          fetch(url + "/api/data/machines?size=" + Number.MAX_SAFE_INTEGER + "&hash=" + sessionStorage.getItem('logintoken'))
          .then(response => response.json())
          .then(data => {
              data.forEach(element => {
                  var htmlString = `<li class="list-group-item list-group-item-action border-0 mb-2">
...
function startEditMachine(selectedButton){
  lastSelectedButton = selectedButton
  fetch(url + "/api/machine/getMachine?id=" + selectedButton + "&hash=" + sessionStorage.getItem('logintoken'))
  .then(response => response.json())
  .then(data => {
      machineFullData = data
      console.log(data)
